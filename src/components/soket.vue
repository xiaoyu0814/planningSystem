...
export default {
  ...
  methods: {
    ...
    initWebSocket() {
      ...
      const WS_URI = pusher_webSoket + "/ws";
    ...
    websocketOnMessage(e) {
      ...
      // console.log(e.data[e.data.indexOf("Text") + 7])
      if (e.data[e.data.indexOf("Text") + 7] == "\\") {
        return;
      }
      // if (e.data[e.data.indexOf("Text") + 7] != "{" && e.data[e.data.indexOf("Text") + 7] != "\"") {
      //   return;
      // }
      ...
      if (data.Type == 1) {
        ...
        if (textData) {
          ...
          } else if (textData.Type == 5) {
            var a = JSON.parse(textData.Data);
