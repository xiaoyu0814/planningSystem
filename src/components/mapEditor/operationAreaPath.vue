<template>
  <div id="operationAreaPath" ref="box" @mousedown.stop="mouseDownHandleelse($event)"
    @mouseup="mouseUpHandleelse($event)">
    <header>
      <span>操作编辑</span>
      <i class="el-icon-close" @click="$store.state.operationAreaPath_show = false"></i>
    </header>
...
export default {
  ...
  methods: {
    ...
    clearNav() {
      ...
      if (geoData) {
        let areaId = geoData.getOperationAreaId();
    ...
    startDraw() {
      ...
      drawLine.startDraw(
        ...
        function (polylayer) {
          ...
          let geoData = data.getGeoData();
          if (geoData) {
            geoData.setOperationAreaId(_areaPath.getNum());
          } else {
            let _geoData = new GeoData();
            _geoData.setOperationAreaId(_areaPath.getNum());
    ...
    importPath() {
      let operationAreaPath = CGFListLoad.datamodel.areaPathNodes;
      // console.log(operationAreaPath);
      this.changeNavPath_show = true;
      if (operationAreaPath) {
        this.navPath_list = [...operationAreaPath.getList()];
      }
    },
    exportPath() {
    ...
    changeNavPath() {
      ...
      // this.tableData = [];
      this.navPath_list = [];
      let operationAreaPath = CGFListLoad.getAreaPath(this.areaPath_value);
      ...
      let geoData = data.getGeoData();
      if (geoData) {
        geoData.setInterestAreaId(operationAreaPath.getNum());
      } else {
        let _geoData = new GeoData();
        _geoData.setInterestAreaId(operationAreaPath.getNum());
        data.setGeoData(_geoData);
      }
